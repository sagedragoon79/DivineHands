...
namespace DivineHands.Modules
{
    ...
    /// RESOURCE NODE (ore MineralDeposit / clay-sand-stone surface patch). Everything else — villagers,
    /// animals, crop fields, dropped items (all share inputManager.selectedObject) — is a safe no-op.
    ///
    ...
    ///   • Pit resource → Object.Destroy(go): its OnDestroy self-unregisters from the mineral manager.
    ///
        ...
        public static string DeleteCurrent()
        {
            ...
            try
            {
                ...
                switch (kind)
                {
                    ...
                    case "pit":      { string n = Name(go); UnityEngine.Object.Destroy(go); return $"Deleted {n}"; }
