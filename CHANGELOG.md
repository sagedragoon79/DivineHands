...
- **Cursor Spawners** — spawn at the cursor, armed in the panel: **Animals** (deer/bear/boar/wolf), **Minerals** (gold/iron/coal as depleting deposits with an optional Deep/infinite toggle; stone/clay/sand always as infinite pits — no finite-stone bug), **Villagers** (immigration loop, never CheatManager), and **Resources** (forageable/tree/rock/giant-rock; **trees auto-plant the map's own species** from `Terrain2.TreePrototypes` with no GUIDs needed — like Pangu; rocks/forageables/giant-rocks use prefab GUID prefs, DLC-null-guarded). Count 1–50 scatters in a ring so spawns don't stack. The panel uses one shared armed-tool state, so arming the spawner disarms the terrain brush and vice-versa.
